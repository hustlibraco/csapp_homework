 ...
 * <= -1，则 f = -1。否则，f=0。
 *
 * 对于正数的情况，两个整数的小数和就是右移溢出位的和。
 * 例如 0.1 + 0.101 >= 1 可以等价计算 100 + 101 >= 1000
 *
 * 对于负数的情况，小数位的绝对值可以用取补(~x+1=-x)的方式得到。
 * 例如 10.101的小数位等于~(101)+1=011=3>>3=0.375, 此时 0.1 + 0.101 >= 1 等价 100 + 011 >= 1000
 * 当小数和绝对值>=1时, neg_bias = 1, 所以负数的时候采用的减法
 * 另外注意两点：
 * 1.求补的时候要注意只取低位，所以要和相应的掩码求与。
 * 2.负数模拟向零舍入的整数除法需要加上偏置值。所以计算整数部分时，pos_base和neg_base不一样
 */
int fiveeights(int x) {
	int w = sizeof(int) << 3;
	int sign_mask = (x & INT_MIN) >> w - 1;
	int pos_base = (x >> 1) + (x >> 3);
	int neg_base = (x + 1 >> 1) + (x + 7 >> 3);
	int pos_bias = (((x & 1) << 2) + (x & 7)) >> 3;
	int neg_bias = ((-(x & 1) & 1) << 2) + (-(x & 7) & 7) >> 3;
	return (sign_mask & neg_base - neg_bias) | (~sign_mask & pos_base + pos_bias);
}
